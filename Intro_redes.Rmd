...
output: pdf_document
---
Carga las paqueterías a utilizar
igraph,tidyverse,tidygraph,visNetwork,networkD3,RColorBrewer,ggraph
...
Ejemplo sencillito
Llama al entorno el csv que se llama prueba_redes y graficalo
```{r}
...
Llamar el archivo Lista_nodosQro2.csv al entorno
```{r}
...
```
Hacer una tabla de frecuencias que se llame frecuencias.Qro
```{r}
...
plot(routes_igraph,layout=layout_with_graphopt, vertex.color=FloColors,vertex.radius=1.5, edge.lwd=.8)
```
