...
```{r}
library(pacman)
p_load(igraph,tidyverse,tidygraph,visNetwork,networkD3,RColorBrewer,ggraph)
```
...
Ejemplo sencillito
```{r}
prueba_redes<-read_csv("prueba_redes.csv", col_names=FALSE)
g<-graph.data.frame(prueba_redes,directed = F)
plot(g)
```
